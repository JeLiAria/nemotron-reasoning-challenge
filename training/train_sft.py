...
# v9.0: load TINKER_API_KEY from the key file if not already in the environment.
# (train_sft.py's tinker client only reads the env var, not a file.)
# Portable for release: set TINKER_API_KEY directly, or point TINKER_API_KEY_FILE
# at a file containing the key. (Originally a hard-coded local path.)
_KEY_FILE = os.environ.get("TINKER_API_KEY_FILE", "")
if not os.environ.get("TINKER_API_KEY") and _KEY_FILE and os.path.exists(_KEY_FILE):
    os.environ["TINKER_API_KEY"] = open(_KEY_FILE, encoding="utf-8").read().strip()
...
class Cfg:
    ...
    lr_schedule: LRSchedule = dataclasses.field(
        # v9.0: 5e-5 (4x lower than Tong's 2e-4). Our adapters were 5-8x stronger
        # than Tong's (||dW|| measured); the high LR drove global over-fitting
        # (loss -> 0.003). Lower LR pulls magnitudes toward Tong's gentle endpoint.
        default_factory=lambda: StepLinearDecayLRSchedule(learning_rate=5e-5)
    )
...
def filter_training_examples(examples: list[TrainingExample]) -> list[TrainingExample]:
    # v9.0: train on the FULL corpus. Tong had left a debug filter here that
    # restricted training to category == "spelling" (648 examples only) — that
    # would have crippled the run. Use all included examples.
    return examples
    # --- previous debug filters (kept for reference) ---
    # return [e for e in examples if e.category in ("spelling")]
    ...
    with (
        ...
        for epoch in range(cfg.num_epochs):
            ...
            for batch_indices in batches:
                ...
                # v9.0 early-stop signal: print loss/token so we can spot the
                # ~0.03 sweet-spot. Do NOT let this fall to ~0.003 (memorization
                # — that is exactly what cost us the regression).
                _v9_loss = metrics_record.get("_loss_per_token")
                if _v9_loss is not None:
                    logger.info(f"[v9] step={step} loss_per_token={_v9_loss:.4f}"
                                + ("  <-- near early-stop target 0.03"
                                   if _v9_loss <= 0.035 else ""))
                ...
                # v9.0: intermediate checkpoints for early-stop. Tong only saves
                # "final"; we want to pick the step where loss~0.03 (before
                # over-fitting). ~12 checkpoints over ~280 steps.
                V9_SAVE_EVERY = 25
                if step % V9_SAVE_EVERY == 0:
                    await training_client.save_checkpoint_async(
                        name=f"step{step}",
                        log_path=str(log_path),
                    )
                    logger.info(f"[v9] saved checkpoint step{step}")
